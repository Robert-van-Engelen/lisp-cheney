...
Like [tinylisp](https://github.com/Robert-van-Engelen/tinylisp), this Lisp interpreter's memory is a `cell[N]` array of Lisp expressions `L`.  With a copying garbage collector we need two `heap[2]` of `N` cells each.  We also use a `cell` pointer pointing to the active cells and a `from` pointer to move cells between heaps in the garbage collection phase:
