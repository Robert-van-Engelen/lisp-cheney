...
I've documented this project's source code extensively to explain the inner workings of the Lisp interpreter, which should make it easy to use and to modify the code.  This small Lisp interpreter includes a copying garbage collector that is efficient.  Cheney's [copying garbage collector](https://en.wikipedia.org/wiki/Cheney%27s_algorithm) uses two heaps.  Cells, atoms and strings on the heap are moved by the copying garbage collector to make space.  This requires registering C variables that contain Lisp values (e.g. the local variables of a C function that reference Lisp values on the heap).  When `n` variables `x1` to `xn` are registered with `var(n, &x1, &x2, ...&xn)` with the garbage collector to update, then these variables are automatically updated to reference the moved cells, atoms and strings on the heap.  A C function must return with `return ret(n, <ret-value>)` to de-register `n` variables and return `<ret-value>`.
